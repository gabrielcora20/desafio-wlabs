...
namespace Wlabs.Services.AwesomeApi.Controllers
{
    [Authorize]
    [Route("[controller]")]
    public class LocalizacaoController : ApiController
    {
        ...
        {
            return !ModelState.IsValid ? CustomResponse(ModelState) : CustomResponse(await _localizacaoAppService.ConsultaAwesomeApi(cep));
        }
    }
}
